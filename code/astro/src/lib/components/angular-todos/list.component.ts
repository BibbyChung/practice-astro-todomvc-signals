...
export class ListComponent {
  ...
  todos$ = getTodos()
  checkBoxElemRef = viewChild.required<ElementRef<HTMLInputElement>>('checkBoxElemRef')
  ...
  checkSelectAllSub = this.checkSelectAllBtn$
    .pipe(
      takeUntilDestroyed(),
      switchMap(() => toObservable(this.checkBoxElemRef, { injector: this.injector })),
      switchMap((elemRef) => setAllTodosCompleted(elemRef.nativeElement.checked))
    )
    .subscribe()
